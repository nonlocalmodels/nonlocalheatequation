...
// A small square part of the original space whose temperature values will be
// computed on a single thread
class partition_space {
 public:
  ...
  explicit partition_space() : data_(new double[0]), size_(0), gx(0), gy(0) {}
  ...
  partition_space(long nx, long ny, long gx, long gy)
      : data_(new double[nx * ny]), size_(nx * ny), gx(gx), gy(gy) {
...
// partitioned data available for threads for reading only for efficient
// accesses and remove the need for unordered_map in the indexing part
...
class solver {
 public:
  ...
  solver(long nt, long nd, long nlog, bool test) {
    ...
    for (space_2d_fut &s : S) s.resize(np * np);

    // resizing the global temperature grid for quick access for threads
    for (space_2d &t : temperature_grid) t.resize(np * np);

    auto range = boost::irange((long)0, np * np);
    hpx::parallel::for_each(
        hpx::parallel::execution::par, std::begin(range), std::end(range),
        [this](std::size_t i) {
          this->S[0][i] =
              hpx::make_ready_future(partition_space(nx, ny, i % np, i / np));
  ...
  void compute_l2(long time) {
    ...
    for (long sx = 0; sx < nx * np; ++sx)
      for (long sy = 0; sy < ny * np; ++sy)
        error_l2 += std::pow(
            S[next][grid_loc(sx, sy)].get()[mesh_loc(sx, sy)] - w(sx, sy, time),
            2);
  }
  ...
  void compute_linf(long time) {
    ...
    for (long sx = 0; sx < nx * np; ++sx)
      for (long sy = 0; sy < ny * np; ++sy)
        error_linf = std::max(
            std::abs(S[next][grid_loc(sx, sy)].get()[mesh_loc(sx, sy)] -
                     w(sx, sy, time)),
            error_linf);
  }
  ...
  void print_error(bool cmp) {
    ...
    if (cmp)
      for (long sx = 0; sx < nx * np; ++sx)
        for (long sy = 0; sy < ny * np; ++sy)
          std::cout << "Expected: " << w(sx, sy, nt) << " Actual: "
  ...
  static inline double boundary(long pos_x, long pos_y, long next) {
    if (pos_x >= 0 && pos_x < nx * np && pos_y >= 0 && pos_y < ny * np) {
      return temperature_grid[next][grid_loc(pos_x, pos_y)]
                             [mesh_loc(pos_x, pos_y)];
    } else {
      return 0;
    }
  }
    ...
    for (long sx = pos_x - eps; sx <= pos_x + eps; ++sx) {
      ...
      for (long sy = pos_y - len_line; sy <= pos_y + len_line; ++sy) {
        result_local += influence_function(distance(pos_x, pos_y, sx, sy)) *
                        c_2d * (boundary(sx, sy, current) - pos_val) *
                        (dh * dh);
      }
    }
  ...
  static partition_space sum_local_partition(
      partition_space middle,
      const std::vector<partition_space> &neighbour_squares, long time,
      bool test) {
    long gx = middle.gx, gy = middle.gy;
    partition_space &next =
        temperature_grid[(time + 1) % 2][get_loc(gx, gy, np)];
    ...
    for (long sx = 0; sx < nx; ++sx) {
      for (long sy = 0; sy < ny; ++sy) {
        next[get_loc(sx, sy)] =
            middle[get_loc(sx, sy)] +
            (sum_local(sx + gx * nx, sy + gy * ny, time % 2) * dt);
        if (test)
          next[get_loc(sx, sy)] +=
  ...
  // do all the work on 'nx * ny' data points for each of 'np * np' partitions
  // for 'nt' time steps
  void do_work() {
    ...
    for (long t = 0; t < nt; ++t) {
      ...
      if ((t % nd) == 0) {
        S[next][0].then([&sem, t](partition_fut &&) {
      ...
      if (t % nlog == 0)
        hpx::dataflow(hpx::launch::async,
                      hpx::util::unwrapping(&solver::log_csv_vtk), S[next], P,
                      t, test);
    }
...
int hpx_main(hpx::program_options::variables_map &vm) {
  ...
  std::uint64_t nd =
      vm["nd"].as<std::uint64_t>();  // Depth for which dependency tree is
                                     // allowed to grow
  std::uint64_t nlog =
      vm["nlog"]
  ...
  if (vm.count("test_batch")) return batch_tester(nd, nlog);
