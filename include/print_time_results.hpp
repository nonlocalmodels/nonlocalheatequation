...
void print_time_results(std::uint32_t num_localities,
                        std::uint64_t num_os_threads, std::uint64_t elapsed,
                        std::uint64_t nx, std::uint64_t ny, std::uint64_t npx,
                        std::uint64_t npy, std::uint64_t nt, bool header) {
  if (header)
  ...
  std::string const npx_str = hpx::util::format("{},", npx);
  std::string const npy_str = hpx::util::format("{},", npy);
  std::string const nt_str = hpx::util::format("{} ", nt);

  hpx::util::format_to(
      std::cout, "{:-6} {:-6} {:.14g}, {:-21} {:-21} {:-21} {:-21} {:-21}\n",
      locs_str, threads_str, elapsed / 1e9, nx_str, ny_str, npx_str, npy_str,
      nt_str)
      << std::flush;
}
...
void print_time_results(std::uint64_t num_os_threads, std::uint64_t elapsed,
                        std::uint64_t nx, std::uint64_t ny, std::uint64_t np,
  ...
  hpx::util::format_to(
      std::cout, "{:-21} {:.14g}, {:-21} {:-21} {:-21} {:-21}\n", threads_str,
      elapsed / 1e9, nx_str, ny_str, np_str, nt_str)
      << std::flush;
}

void print_time_results(std::uint64_t num_os_threads, std::uint64_t elapsed,
                        std::uint64_t nx, std::uint64_t ny, std::uint64_t nt,
                        bool header) {
  if (header)
    std::cout << "OS_Threads,       Execution_Time_sec,"
  ...
  hpx::util::format_to(std::cout,
                       "{:-21} {:10.12},        {:-21} {:-21} {:-21}\n",
                       threads_str, elapsed / 1e9, nx_str, ny_str, nt_str)
      << std::flush;
}
